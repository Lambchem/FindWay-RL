...
class VecGridWorldFast:
    def __init__(self, n_envs, size=10, obstacle_p=0.2, max_steps=250, base_seed=1234):
        """
        obstacle_p: float  → 所有环境使用同一密度
                    (low, high) → 每个环境在 [low, high] 内均匀随机采样密度，
                                  每局结束重置时重新采样，实现课程随机化
        """
        self.N          = n_envs
        self.size       = size
        ...
        # 支持固定值或范围
        if isinstance(obstacle_p, (list, tuple)):
            self._op_low, self._op_high = float(obstacle_p[0]), float(obstacle_p[1])
            self._op_fixed = None
        else:
            self._op_low = self._op_high = float(obstacle_p)
            self._op_fixed = float(obstacle_p)
        # 每个环境的当前 obstacle_p
        self.obstacle_ps = self._sample_ops()
    ...
    def _sample_ops(self):
        """为所有环境采样一次 obstacle_p"""
        if self._op_fixed is not None:
            return np.full(self.N, self._op_fixed, dtype=np.float32)
        return np.random.uniform(self._op_low, self._op_high, size=self.N).astype(np.float32)
    ...
    def _new_grid(self, i):
        rng, n, p = self._rngs[i], self.size, float(self.obstacle_ps[i])
        while True:
    ...
    def step(self, actions):
        ...
        # 只重置完成的环境；同时重新采样该环境的 obstacle_p，增加多样性
        for i in np.where(dones)[0]:
            if self._op_fixed is None:
                self.obstacle_ps[i] = np.random.uniform(self._op_low, self._op_high)
            self.grids[i] = self._new_grid(i)
            self.pos[i]   = 0
            self.steps[i] = 0
...
if __name__ == "__main__":
    SIZE = 10
    # 混合训练：障碍密度在 20%~40% 之间均匀随机，每局独立采样
    # 在原 20% checkpoint 基础上继续，不会遗忘简单地图的能力
    OB_P = (0.20, 0.40)
    ...
    model = train_ppo_vec(
        size=SIZE,
        obstacle_p=OB_P,      # 传入范围元组
        max_steps=250,
        n_envs=4096,
        horizon=512,
        total_updates=5000,    # 在原 1200 轮基础上继续到 1800
        lr=1e-4,              # 继续训练用更小学习率，避免破坏已有策略
        gamma=0.99,
        lam=0.95,
        clip=0.2,
        vf_coef=0.5,
        ...
        minibatch_size=8192,
        save_every=100,
        resume=True,          # 从上次 checkpoint 继续
        device=device,
    )

    print("训练完成：开始 pygame 演示（关闭窗口退出）")
    demo_pygame(model, size=SIZE, obstacle_p=0.30, episodes=30, fps=14, device=device)